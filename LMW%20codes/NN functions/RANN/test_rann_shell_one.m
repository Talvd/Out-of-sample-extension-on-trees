...
function test_rann_shell_one(n,m,k,nit,isuper,istat)

file_diary = ['diary_rann_shell_one_n', int2str(n), '_m', int2str(m), ...
    '_k', int2str(k)];
delete(file_diary);
diary(file_diary);
...
% run algorithm
disp('Running rann_shell_one...');
told = now;
disp(['Started at ', datestr(now)]);

shell_str = ['./rann_shell_one ', int2str(n), ' ', int2str(m), ' ', int2str(k), ...
...
if (ier == 0)
    ...
    disp('Start saving idx_one, dst_one...');
    file_idx = 'idx_one';
    file_dst = 'dst_one';
