...
/*
  ...
  2) Call ier = unix('rann_shell_one 5 3 2 3 1 1'), where ier = error code
...
void main(int argc, char **argv)
{
  ...
  if (argc != 7) {
    printf("usage: rann_shell_one n m k numit isuper istat\n");
    exit(1);
  }
  ...
  nwords = 0;
  get_memory_size_(&n, &m, &k, &numit, &isuper, &nwords);
  ...
  /* REPORT */
  FILE *report_file;
  report_file = fopen("rann_report_one", "a");
